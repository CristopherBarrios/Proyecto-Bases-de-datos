...
<form action="insertar.php" method="POST" enctype="multipart/form-data">
				<table class="table">
				<tr><th colspan="5" class="bg-primary text-center" >AGREGA NUEVAS PISTAS DE AUDIO [DOCUMENTALES]</th></tr>
				<tr class="bg-primary">
				<th>PORTADA</th>
				<th>NOMBRE</th>
				<th>AUDIO</th>
				...
				<td><input  name="submitdoc" type="submit" class="btn btn-success" value="AGREGAR" > </td>
				</tr>
				...
				<tr class="bg-info">
						<td><div style="text-align:right;">
							<input  name="elimdoc" type="submit" class="btn btn-default" value="ELIMINAR" onclick='return confirm("¿Seguro que deseas eliminar este registro? Se borraran TODOS los registros de esta categoria (portadas, audios, etc)")'  >
						</div>
...
<option hidden value="DOCUMENTALES">→→→CATEGORIAS←←←</option>

	<?php
include('class.php');
	$res2 =  mysql_query("SELECT * FROM progra3 ORDER BY nombre ASC");
...
<?php } ?>
				...
				<form  action="programadoc.php" method="POST"><td>
				<input required class="btn " type="text" name="nuevo" placeholder="Nueva Categoría...">
...
$sql = "SELECT * FROM documentales ORDER BY name ASC";

$res =  mysql_query($sql);

$sqli = "SELECT * FROM progra3";
...
$fecha2=date("d-m-Y",strtotime($fecha1));
					echo "<tr>
							...
							<td>".$fecha2. "</td>
							<td><form action='eliminardoc.php?id=".$row['id']."' method='POST' enctype='multipart/form-data'>
							...
							<form action='modificardoc.php?id=".$row['id']."'method='POST' enctype='multipart/form-data'>
